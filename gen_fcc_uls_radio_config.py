...
def load_dat_to_sqlite(conn, filepath, table, new_db=False):
    ...
    if (not table_exists(conn, table)):
        ...
        if (not new_db):
            print(f"Table {table} exists. Using existing table and row rata. Use -cc or --clearcache to clear SQL tables")    
            return
        
        print(f"Table {table} exists. Using existing table and appending row data. Use -cc or --clearcache to clear SQL tables")
...
def search_frequencies(conn, zip_codes=None, city=None, service_codes=None, status='active'):
    cursor = conn.cursor()
    try:
        ...
        #This seems to get rid of the duplicates in the EM table
        #The only value different in the EM rows returned I've found so far being EM col_3 (ULS File Number)?
        #Have not tested potential ramifications of missing results due to this 
        #Like possibly missing multiple results for the same frequency 
        #with the same USI number with active and expired statuses?
        #Or other scenarious
...
def main():
    ...
    parser.add_argument('-ls', '--listservices', action='store_true', help="List available radio service code(s) to search")
    parser.add_argument('-s', '--service', help="Radio service code(s) to search, comma-separated (e.g. PW,IG)")
    parser.add_argument('--status', choices=['active', 'expired', 'any'], default='active', help="License status filter, default is active")
    parser.add_argument('-lz', '--listzipfiles', action='store_true', help="List available ZIP files to download from FCC")
    parser.add_argument('-zf', '--zipfiles', help="Comma-separated ZIP filenames to download and load (e.g. l_LMpriv.zip,l_AM.zip). Default is l_LMpriv.zip")
    parser.add_argument('-cc', '--clearcache', action='store_true', help="Clear cached ZIP files and SQL tables. Default is to use cached data if it exist")
    ...
    if (os.path.exists(DB_FILE)):
        if (args.clearcache):
            print(f"{DB_FILE} exists, but re-creating database because --clearcache was specified")
            os.remove(DB_FILE)
        else:
            print(f"{DB_FILE} exists. Using existing database. Use -cc or --clearcache to re-create database")
    ...
    for zip_filename in zip_filenames:
        ...
        if os.path.exists(zip_filename_full_path):
            if args.clearcache:
                print(f"{zip_filename} exists, but re-downloading because --clearcache was specified")
                download_with_progress(zip_url, DATA_DIR + '/' + zip_filename)
            else:
                print(f"{zip_filename} exists. Using existing file. Use -cc or --clearcache to re-download ZIP files")
        else:
        ...
        if not os.path.exists(extract_dir):
            ...
            print(f"{extract_dir} directory exists, but re-extracting because --clearcache was specified")
            extract_zip(zip_filename_full_path, extract_dir)
        else:
            print(f"{extract_dir} directory exists. Skipping extraction. Use -cc or --clearcache to re-extract ZIP files")
